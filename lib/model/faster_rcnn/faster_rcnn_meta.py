...
import time
import pdb
from model.faster_rcnn.layers import linear
from model.utils.net_utils import _smooth_l1_loss, _crop_pool_layer, _affine_grid_gen, _affine_theta


class _fasterRCNNMeta(nn.Module):
    """ faster RCNN """

    def __init__(self, classes, class_agnostic):
        super(_fasterRCNNMeta, self).__init__()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, RCNN_top_weight=None, RCNN_cls_score_weight=None, RCNN_bbox_pred_weight=None):
            #vars = self.RCNN_rpn.vars + self.RCNN_top.vars + self.RCNN_cls_score.vars+self.RCNN_bbox_pred.vars
        ...
        pooled_feat = self.RCNN_top(pooled_feat, RCNN_top_weight).mean(3).mean(2)

        # compute bbox offset
        if RCNN_bbox_pred_weight is None:
            bbox_pred = self.RCNN_bbox_pred(pooled_feat)
        else:
            bbox_pred = linear(pooled_feat, RCNN_bbox_pred_weight[0], RCNN_bbox_pred_weight[1])
        ...
        if RCNN_cls_score_weight is None:
            cls_score = self.RCNN_cls_score(pooled_feat)
        else:
            cls_score = linear(pooled_feat, RCNN_cls_score_weight[0], RCNN_cls_score_weight[1])
