...
from model.faster_rcnn.faster_rcnn_hierarchy_attention import _hierarchyAttentionFasterRCNN
...
class PreResNet50HierarchyCasecade(_hierarchyAttentionFasterRCNN):

    def __init__(self, main_classes, sub_classes, pretrained=False, class_agnostic=False, weight_file=None, fixed_layer=0, casecade_type='add_score', alpha=0.5):
        ...
        _hierarchyAttentionFasterRCNN.__init__(
            self, main_classes, sub_classes, class_agnostic, casecade_type, alpha)
    ...
    def _init_modules(self):
        ...
        self.RCNN_cls_score_main = nn.Linear(1024, self.n_main_classes)
        self.RCNN_cls_score_sub = nn.Linear(1024, self.n_sub_classes)

        if self.class_agnostic:
            self.RCNN_bbox_pred_main = nn.Linear(1024, 4)
            self.RCNN_bbox_pred_sub = nn.Linear(1024, 4)
        else:
            self.RCNN_bbox_pred_main = nn.Linear(1024, 4 * self.n_main_classes)
            self.RCNN_bbox_pred_sub = nn.Linear(1024, 4 * self.n_sub_classes)
