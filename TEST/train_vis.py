...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            roi_data = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)

            pdb.set_trace()
            rois, rois_label, rois_target, rois_inside_ws, rois_outside_ws = roi_data

            im2show = im_data.cpu().numpy()
            im_info_numpy = im_info.cpu().numpy()
            scale = im_info_numpy[0, 2]
            boxes = rois[0, :, :].cpu().numpy() * scale
            boxes[:, 0] = 1
            boxes = np.transport(boxes, (1, 2, 3, 4, 0))

            im2show = vis_detections(
                im2show, imdb.classes[0], boxes, 0.003)
            # backward
